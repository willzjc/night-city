...
test('mouse capture turns the camera and Escape releases it', async ({ page, isMobile }) => {
  ...
  await test.step('Deliver explicit relative motion independently of the headless OS cursor', async () => {
    await page.locator('#city').dispatchEvent('mousemove', { movementX: 60, movementY: 20 })
    const turned = await snapshot(page)
    expect(turned.yaw).toBeCloseTo(captured.yaw - 0.12, 5)
    expect(turned.pitch).toBeCloseTo(captured.pitch - 0.04, 5)
  })
  ...
  expect(idle.yaw).toBe(released.yaw)
  expect(idle.pitch).toBe(released.pitch)
})
